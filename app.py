...
top_anime=(df.sort_values(by=[ 'members','rating'], ascending= False)).head(10)
...
def recommendation(anime):
    ...
    recommendations = NearestNeighbors(n_neighbors=6, algorithm='ball_tree').fit(Anime_final)
    recommendations.kneighbors(Anime_final)
    anime_indices = recommendations.kneighbors(Anime_final)[1] 
    #print('Here are anime similar to', anime, ':' '\n')
    #index = obtain_animeindex(anime)
    index=df[df.name == anime].index.tolist()[0]
    ...
    return pd.DataFrame(Data, columns=['Name','Genre','Rating'], index=['1','2','3','4','5'])
